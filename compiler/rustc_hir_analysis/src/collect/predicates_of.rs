...
pub(super) fn implied_predicates_with_filter<'tcx>(
    ...
    icx.lowerer().adjust_sizedness_predicates(&mut bounds, superbounds, item.span);
...
impl<'tcx> ItemCtxt<'tcx> {
    ...
    fn probe_ty_param_bounds_in_generics(
        ...
        for predicate in hir_generics.predicates {
            ...
            self.lowerer().adjust_sizedness_predicates(
                &mut bounds,
                predicate.bounds,
                predicate.bounded_ty.span,
            );
        }
...
pub(super) fn const_conditions<'tcx>(
    ...
    if let Some((def_id, supertraits)) = trait_def_id_and_supertraits {
        ...
        icx.lowerer().adjust_sizedness_predicates(&mut bounds, supertraits, DUMMY_SP);
    }
