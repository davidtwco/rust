...
impl<'tcx> dyn HirTyLowerer<'tcx> + '_ {
    ...
    pub(crate) fn adjust_sizedness_supertraits(
        ...
        if trait_did == pointeesized_did {
            // Never add a default supertrait to `MetaSized` or `PointeeSized` - would defeat the
            // point somewhat.
            return;
        }
            ...
            hir::AssocItemConstraintKind::Bound { bounds: hir_bounds } => {
                match predicate_filter {
                    ...
                    | PredicateFilter::ConstIfConst => {
                        ...
                        self.adjust_sizedness_predicates(bounds, hir_bounds, constraint.span);
                    }
                    PredicateFilter::SelfOnly
                    | PredicateFilter::SelfTraitThatDefines(_)
