...
/// (`Sized`, `MetaSized` or `PointeeSized`).
...
fn sizedness_constraint_for_ty<'tcx>(
    ...
    match ty.kind() {
        // Always `Sized`, `MetaSized` or `PointeeSized`
        Bool
        | Char
        | Int(..)
        | Uint(..)
        ...
        Str | Slice(..) | Dynamic(_, _, ty::Dyn) => match sizedness {
            ...
            // Always `MetaSized` and `PointeeSized`
            Sizedness::MetaSized | Sizedness::PointeeSized => None,
        },

        // Maybe `Sized`, `MetaSized` or `PointeeSized`
        Param(..) | Alias(..) | Error(_) => Some(ty),

        Foreign(..) => match sizedness {
            // Never `MetaSized` or `Sized`
            Sizedness::Sized | Sizedness::MetaSized => Some(ty),
            // Always `PointeeSized`
            Sizedness::PointeeSized => None,
        },
        ...
        Adt(adt, args) => {
            let constraint = match sizedness {
                ...
                Sizedness::PointeeSized => adt.pointeesized_constraint(tcx),
            };
...
///
/// Calculates the `PointeeSized` constraint.
///
/// In fact, there are only a few options for the types in the constraint:
///     - a type parameter or projection whose sizedness can't be known
#[instrument(level = "debug", skip(tcx), ret)]
fn adt_pointeesized_constraint<'tcx>(
    tcx: TyCtxt<'tcx>,
    def_id: DefId,
) -> Option<ty::EarlyBinder<'tcx, Ty<'tcx>>> {
    adt_sizedness_constraint(tcx, Sizedness::PointeeSized, def_id)
}
...
fn adt_sizedness_constraint<'tcx>(
    ...
    let lang_item = match sizedness {
        ...
        Sizedness::PointeeSized => LangItem::PointeeSized,
    };
