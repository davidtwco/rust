...
{
    match ty.kind() {
        // impl {Meta,}Sized for u*, i*, bool, f*, FnDef, FnPtr, *(const/mut) T, char, &mut? T
        // impl {Meta,}Sized for [T; N], dyn* Trait, !, Coroutine, CoroutineWitness, Closure
        // impl {Meta,}Sized for CoroutineClosure
        ...
        ty::Str | ty::Slice(_) | ty::Dynamic(..) => match sizedness {
            ...
            Sizedness::MetaSized => Ok(vec![]),
        },

        ty::Foreign(..) | ty::Alias(..) | ty::Param(_) | ty::Placeholder(..) => Err(NoSolution),
        ...
        ty::Adt(def, args) => {
            ...
            if let Some(crit) = crit {
                Ok(vec![ty::Binder::dummy(crit.instantiate(ecx.cx(), args))])
            } else {
                Ok(vec![])
            }
        }
