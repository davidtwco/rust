...
impl<'tcx> AutoTraitFinder<'tcx> {
    ...
    fn evaluate_predicates(
        ...
        for predicate in param_env.all_clauses(infcx.tcx) {
        ...
        let mut user_computed_preds: FxIndexSet<_> =
            user_env.all_clauses(infcx.tcx).map(|c| c.as_predicate()).collect();
        ...
        while let Some(pred) = predicates.pop_front() {
            ...
            let computed_preds = param_env
                .all_clauses(infcx.tcx)
                .map(|c| c.as_predicate())
                .chain(user_computed_preds.iter().cloned());
            let normalized_preds = elaborate(tcx, computed_preds);
            new_env = ty::ParamEnv::from_clauses_iter(
                tcx,
                normalized_preds.filter_map(|p| p.as_clause()),
            );
        }

        let final_user_env = ty::ParamEnv::from_clauses_iter(
            tcx,
