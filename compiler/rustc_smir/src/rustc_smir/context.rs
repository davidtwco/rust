...
impl<'tcx> Context for TablesWrapper<'tcx> {
    ...
    fn try_new_const_zst(&self, ty: Ty) -> Result<MirConst, Error> {
        ...
        let size = tables
            .tcx
            .layout_of(ty::TypingEnv::fully_monomorphized(tables.tcx).as_query_input(ty_internal))
            .map_err(|err| {
                Error::new(format!(
    ...
    fn try_new_const_uint(&self, value: u128, uint_ty: UintTy) -> Result<MirConst, Error> {
        ...
        let size = tables
            .tcx
            .layout_of(ty::TypingEnv::fully_monomorphized(tables.tcx).as_query_input(ty))
            .unwrap()
            .size;
