...
impl<'tcx> Ty<'tcx> {
    ...
    #[instrument(skip(tcx), level = "debug")]
    pub fn has_trivial_const_sizedness(self, tcx: TyCtxt<'tcx>) -> bool {
        match self.kind() {
            ty::Infer(ty::IntVar(_) | ty::FloatVar(_))
            | ty::Uint(_)
            | ty::Int(_)
            ...
            ty::Tuple(tys) => tys.iter().all(|ty| ty.has_trivial_const_sizedness(tcx)),

            ty::Adt(def, args) => def
                .all_fields()
                .map(|def| def.ty(tcx, args))
                .all(|ty| ty.has_trivial_const_sizedness(tcx)),
