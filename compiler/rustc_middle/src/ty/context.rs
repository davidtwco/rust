...
use std::marker::{MetaSized_, PhantomData};
...
struct InternedInSet<'tcx, T: ?Sized + ?MetaSized_>(&'tcx T);

impl<'tcx, T: 'tcx + ?Sized + ?MetaSized_> Clone for InternedInSet<'tcx, T> {
    fn clone(&self) -> Self {
        InternedInSet(self.0)
    }
}

impl<'tcx, T: 'tcx + ?Sized + ?MetaSized_> Copy for InternedInSet<'tcx, T> {}

impl<'tcx, T: 'tcx + ?Sized + ?MetaSized_> IntoPointer for InternedInSet<'tcx, T> {
