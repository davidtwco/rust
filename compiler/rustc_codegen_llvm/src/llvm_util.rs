            ...
            .filter(|(feature, _, _)| {
                ...
                if let Some(feat) = to_llvm_features(sess, feature) {
                    for llvm_feature in feat {
                        ...
                        if !unsafe {
                            llvm::LLVMRustHasFeature(target_machine.deref(), cstr.as_ptr())
                        } {
                            return false;
                        }
                    }
...
pub(crate) fn print(req: &PrintRequest, out: &mut String, sess: &Session) {
    ...
    match req.kind {
        PrintKind::TargetCPUs => print_target_cpus(sess, tm.deref(), out),
        PrintKind::TargetFeatures => print_target_features(sess, tm.deref(), out),
